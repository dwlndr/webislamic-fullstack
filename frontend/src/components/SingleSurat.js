...
import "./css/quran/SingleSurat.css";
...
const SingleSurat = () => {
  ...
  const getAPIsinglesurat = async () => {
    try {
      setLoading(true);
      const response = await fetch(`https://equran.id/api/surat/${idsurat}`);
      const data = await response.json();
      setSingleSurat(data["ayat"]);
      setDec(data);
    } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="container-surat mx-auto">
        <div className="px-3 lg:px-0 flex flex-wrap gap-4 border-b-[1px] border-slate-300 pb-2 items-end">
          <button className={`btn btn-primary ${isAudioPlaying ? "hidden-button" : ""}`} onClick={handleAudioPlay}>
            Play
          </button>
          <button className={`btn btn-danger ${isAudioPlaying ? "" : "hidden-button"}`} onClick={handleAudioPause}>
            Pause
          </button>
          <audio id="audio-tag" ref={audioRef} src={decSurat.audio}></audio>
        ...
        <div className="flex justify-between my-5 px-3 md:px-0">
          {decSurat.surat_sebelumnya !== false && (
            <a
          ...
          {decSurat.surat_selanjutnya !== false && (
            <a
